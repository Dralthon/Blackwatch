...
for count in range(0, len(encryption1) - 1):
	if encryption1[count] != " ":
		for letters in range(0, len(checkList) - 1):
			for pages in range(0, len(checkList) - 1):
				solution1[letters, pages, count] = checkList[count]

for page in range(0, len(checkList) - 1):
	for row in range(0, len(checkList) - 1):
		for letter in range(0, len(solution1[rows]) - 1):
			if solution1[letter] != " ":
				word = solution1[page, row, letter]
			else:
				wordCount[wordNum] = d.check(word)
				wordNum = wordNum + 1
